...
#define VALUE_MISPREDICT_PENALTY 1
...
public:
  uint32_t cpu;
  uint32_t num_mem_loads;
  uint32_t num_mem_stores;
  ...
  // value prediction
  uint32_t vp_num_reg_executions; // > 4billion instructions?
  uint32_t vp_num_mem_executions;
  ...
  O3_CPU()
  {
    ...
    // value prediction
    vp_num_reg_executions = 0;
    vp_num_mem_executions = 0;
